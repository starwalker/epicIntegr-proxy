...
public class Decoder {
    ...
    protected HashMap<String, String> decodeHeaders(String data) {
        ...
        if (versionNumber <= 1) {
            ...
            try {
                ...
                length = Integer.parseInt(data.substring(27, 30));
            }
        }
    ...
    protected HashMap<String, String> decodeSubFile(String data) {
        ...
        for (String l : lines) {
            if (l.length() > 6) {
                if (l.indexOf("ANSI ", 0) == 0 && l.contains("DL")) {
                    l = l.substring(l.indexOf("DL"));
                    String str1[] = l.split("DL");
                    if (str1.length > 1) {
                        l = str1[str1.length - 1];
                    }
                }
                if (l.indexOf("AAMVA", 0) == 0 && l.contains("DL")) {
                    l = l.substring(l.indexOf("DL"));
                    String str1[] = l.split("DL");
                    if (str1.length > 1) {
                        l = str1[str1.length - 1];
                        if (l.contains("DAQ")) {
                            l = l.substring(l.indexOf("DAQ"));
                        }
                    }
                }
            }

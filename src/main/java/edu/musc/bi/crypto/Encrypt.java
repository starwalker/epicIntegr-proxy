...
public class Encrypt {
    ...
    public final String getMRN(final String encryptedStr) {
        String rst = "";
        if (encryptedStr != null && !encryptedStr.isEmpty()) {
            try {
                if ( encryptedStr.contains("=") ) {
                    String[] strSplit = encryptedStr.split("=");
                    rst = strSplit[1];
                }
    ...
    public String getEncryptedString(final int option)
        ...
        String rst = "";
        if (StringUtils.isBlank(bikeydlpw1)) {
            bikeydlpw1 =
                ConfigProvider.getConfig().getOptionalValue("bi-key-dl-pw1", String.class);
        }
    ...
    public String encrypt(String inputString, byte[] hash, boolean removeUrlEncoding)
        ...
        if (removeUrlEncoding) {
            String inputStringDecoded = URLDecoder.decode(inputString, StandardCharsets.UTF_8.name());
        }
        byte[] input = DatatypeConverter.parseBase64Binary(inputString);

        // Pass empty IV
        byte[] IV = new byte[32];
        // Get instance of appropriate AES cypher
        Cipher aes = Cipher.getInstance("AES");
        aes.init(Cipher.ENCRYPT_MODE, new SecretKeySpec(hash, "AES"), new IvParameterSpec(IV));
    ...
    public byte[] passToAESHash(String pass, int keySize)
        ...
        byte[] concatenatedArray = new byte[hashed36.length + hashed5C.length -1 ];
